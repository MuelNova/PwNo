...
def testAbbr():
    ...
    sl(b'0')
    assert my_sh == sh, f'sh is {sh} with PID {sh.proc.pid} but get_instance() returns {(my_sh_name, my_sh)} with PID {my_sh.proc.pid})'
    sh.close()
...
def testAbbrAfterClose():
    ...
    new_ = get_instance()
    assert new != new_, "get_instance() is still using the old instance without changing it's name"
    sl(b'0')
    another_name.close()
...
def testCustomAbbr():
    ...
    p = gen_sh()
    my_sl(b'2')
    assert p_instance != get_instance('p'), "get_instance(name) doesn't return two different instances."
    p.close()
    sh.close()
